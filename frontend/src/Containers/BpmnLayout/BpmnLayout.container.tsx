...
export const Component = () => {
  ...
  const actions = useMemo(() => {
    ...
    if (isRootFolder) return <FolderActions />;
    if (!resource) return <></>;
  ...
  return (
    <div className="mainContainer">
      ...
      <Sheet className={`folderTree ${folderTreeCollapsed ? 'collapsed' : ''}`}>
        <IconButton
          size="sm"
          variant="plain"
          onClick={() => setFolderTreeCollapsed(!folderTreeCollapsed)}
          sx={{ alignSelf: 'flex-end' }}
          title={folderTreeCollapsed ? 'Afficher l’explorateur' : 'Réduire l’explorateur'}
        >
